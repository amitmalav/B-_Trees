...
char *key; /* returns the key to be filled in the parent */
{
	...
	AM_INTHEADER ihead,itemphead; /* local header */
	AM_INTHEADER *iheader,*itempheader;
...
if (*pageBuf == 'l'){
	...
	//SOME FUNCTION WHICH REMOVES KEY FROM THIS NODE AND SHIFTS ITS RECORDS TO PARENT
...
else { // THE NODE BEING SPLIT IS INTERNAL
	/* compact half the keys into temporary page */
	AM_CompactNode(1,(header->numKeys)/2,pageBuf,tempPage,header);
	...
	/* compact the other half keys */
	AM_CompactNode((header->numKeys)/2 + 1,header->numKeys
			      ,pageBuf,tempPageBuf,header);
