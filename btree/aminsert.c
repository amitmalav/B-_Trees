...
/* THIS NEEDS TO BE CHANGED AS PER THE NEW STRUCTURE*/
...

{
		...
		/* key is already present */ 
	{
		...
		/* THIS PART SHOULD ALSO BE CHANGED ACCORDING TO INTERNAL AND LEAF NODE*/
		/* insert into leaf or internal node  - but no need to split */
	...
	/*there is enough space in the freelist and in the middle put together */
	{
		...
		recSize = attrLength + AM_ss + AM_si;
		if (status == AM_FOUND)
			/* key is already present */ 
		{
			if (iheader->freeListPtr == 0)
				if ((iheader->recIdPtr - iheader->keyPtr) <(AM_si + AM_ss))
				{
					/* no room for one more record */
					return(FALSE);
				}
			...
			/* insert into leaf or internal node  - but no need to split */
			AM_InsertToNodeFound(pageBuf,recId,index,iheader);
			bcopy(iheader,pageBuf,AM_sint);
				return(TRUE);
		}
	}
}
...
// GETS CALLED ONLY WHEN KEY IS FOUND IN INTERNAL NODE
AM_InsertToNodeFound(pageBuf,recId,index,header)
char *pageBuf;
int recId;
int index;
AM_INTHEADER *header;

{
	...
	recSize = header->attrLength + AM_ss + AM_si; // THIS GETS CHANGED AS PER THE NEW INTERNAL NODE STRUCTURE
	if ((header->freeListPtr) == 0)
	{
		header->recIdPtr = header->recIdPtr - AM_si - AM_ss;
		tempPtr = header->recIdPtr;
	...
	/* save the old head of recId list */
	bcopy(pageBuf+AM_sl+AM_si+(index-1)*recSize + header->attrLength,
	      (char *)&oldhead, AM_ss);   // THIS GETS CHANGED VERY MINUTELY

        /* Update the head of recId list to the new recid to be added */
	bcopy((char *)  &tempPtr,pageBuf+AM_sl + AM_si+(index-1)*recSize + 
	       header->attrLength,AM_ss);  // THIS GETS CHANGED VERY MINUTELY
...
AM_CompactNode(low,high,pageBuf,tempPage,header)
...

{
	...
	tempheader = &temphead;
	bcopy(header,tempheader,AM_sint);
	
	recSize = header->attrLength + AM_ss + AM_si;
	recIdPtr = PF_PAGE_SIZE - AM_si - AM_ss ;

	for (i = low, j = 1; i <= high; i++,j++)
	{
		offset1 = (i - 1) * recSize + AM_sint;
		offset2 = (j - 1) * recSize + AM_sint;
		bcopy(pageBuf + offset1,tempPage + offset2,header->attrLength);
		bcopy(pageBuf + offset1 + header->attrLength,(char *)&nextRec
		      ,AM_ss);
		bcopy((char *)&recIdPtr,tempPage + offset2 + header->attrLength,
	...
	tempheader->numKeys = high - low + 1;
	tempheader->maxKeys = header->maxKeys;
	bcopy(tempheader,tempPage,AM_sint);
