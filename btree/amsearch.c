...

{
	...
	while ((**pageBuf) != 'l')
	{
		...
		// BINSEARCH TO BE CHANGED CORRESPONDING TO NEW NODE
		
		nextPage = AM_BinSearch(*pageBuf,attrType,attrLength,value,
					indexPtr,iheader);

		int compareVal = AM_Compare(pageBuf+AM_sl+(*indexPtr - 1)low*recSize,attrType, attrLength, value);
		
		/* NO NEED TO SEARCH FURTHER IF VALUE PRESENT IN INTERNAL NODE*/
		if (compareVal == 0) {
			return AM_FOUND;
		}
...

{
	...
	recSize = AM_si  + attrLength + AM_ss; // AS IT NOW STORES RECORD ID's TOO !
	low = 1;
	high = header->numKeys;
